...
- **基线架构：** 同仓多语言（`apps/frontend / apps/backend / packages/vega-cli / contracts`）。`frontend` 使用 `Vite + React + TypeScript`，`backend` 使用 `Go + Gin`，CLI 使用 `Node.js + commander.js`，数据库采用 `PostgreSQL + JSONB`，鉴权采用 `JWT + bcrypt`，文件存储采用 `Local File System`。
- **顶层编排：** 根目录 `Makefile` 作为**唯一对外命令入口**。Makefile 内部分别调度 `pnpm` (JS 生态) 和 `go` (Go 生态)，以及 `docker`、`playwright` 等异构工具。
- **包管理：** Node.js 部分由 `pnpm workspace` 管理（`apps/frontend` + `packages/vega-cli`）；Go 部分由 `apps/backend/go.mod` 独立管理；两者通过 `contracts/` 桥接，互不干扰。
...
- **归档学习：** 每轮交付后回写 `docs/decisions`、`specs/archive`、`AGENTS.md`，形成持续学习闭环。
...
| 后端 API | `Go` + `Gin` | 简历 CRUD、模板、导出任务、分享接口 | Go 生态最成熟 HTTP 框架，长期维护确定性高；`apps/backend` 独立 `go.mod`，与 JS 栈互不依赖 |
...
- **`vega` CLI**：Node.js 命令行工具，负责状态文件的 CRUD 和阶段校验。Agent 通过 `vega status --json`、`vega transition <phase>` 等命令操作状态。
...
| `specs/archive/` | 归档后的 OpenSpec 四件套 | 永久保留 | 历史可检索范式 |
...
4. **与 pnpm workspace 天然契合**：和 `apps/frontend` 共享 lockfile、tsconfig、lint 规则。

**核心命令清单（与 v3.1 完全对齐）：**

```bash
# 需求管理
vega init <name> [--workflow lite|full]    # 创建需求状态文件
vega status [--json]                       # 查看当前需求状态
vega list                                  # 列出所有需求
vega current                               # 获取当前活跃需求

# 阶段推进
vega transition <phase>                    # 推进到指定阶段
vega complete                              # 标记当前阶段为 completed
vega fail [--reason "..."]                 # 标记当前阶段为 failed

# 模块管理（Full 链路）
vega module add <module-name>              # 添加拆解模块
vega module status <module-name> [--json]  # 查看模块状态
...
# 校验与路由
vega verify                                # 校验状态文件完整性
vega next                                  # 输出下一个应该执行的 Skill 名称
...
| --- | --- | --- | --- |
| `vega-continue` | 任意阶段完成后 | 调用 `vega next` 判断下一步，调用对应 Skill | `vega status --json`、`vega next` |
| `vega-requirement-init` | 拿到新需求时 | 创建状态文件、下载 PRD、设置 workflow | `vega init`、`vega doc set prd <path>` |
...
| `vega-experience` | CI 失败、`vega fail` 执行后或复盘时 | 从失败日志提取根因，更新规则文件 | 无特定 CLI 命令 |
...
1. **当前状态**：执行 `vega status --json` 获取当前需求及模块所处阶段。
2. **近期动作**：执行 `git log -n 5` 了解最近的改动。
3. **关键产物**：读取 `vega status` 返回的 `documents` 字段关联的核心产物（如 PRD、Tech Design 或 OpenSpec）。
...
Agent 执行 continue
    │
    ▼
vega status --json  →  获取 current_phase
    │
    ▼
vega next  →  输出下一个 Skill 名称
    │
    ▼
...
any_phase (failed)             → vega-experience (修复后重置状态并重试当前阶段)
```
...
- **`apps/frontend`**：前端业务应用（Vite + React + TS）。
- **`packages/vega-cli`**：流程工具 CLI（Node.js + commander.js）。
- **`apps/backend`** ：Go + Gin 后端，**不进入 pnpm 治理范围**，独立 `go.mod` 管理。

### 5.2 JS 与 Go 同仓互不干扰原则

1. **依赖隔离**：根目录的 `node_modules` 和 `pnpm-lock.yaml` 仅服务于 JS 项目。`apps/backend` 严禁包含 `package.json`，仅保留 `go.mod` 和 `go.sum`。
2. **构建解耦**：Go 项目的编译与测试完全由 Go 工具链处理。Makefile 通过 `cd apps/backend && go ...` 调用，不让 `pnpm` 介入。
...
3. `pnpm link --global`
4. 在仓库任何目录执行 `vega status` 即可调用本地源码产物，实现"即改即用"。

CI / 团队分发：由于 npm 官方已有 `vega` 包，为防冲突，在 `packages/vega-cli/package.json` 中使用命名空间（如 `"name": "@vega-harness/cli"`），升版后执行 `pnpm publish --access public`（或发内网）。CLI 的暴露命令仍可保持为 `"bin": { "vega": "./bin/index.js" }`。MVP 阶段无需频繁发包，本地链接 + 仓库源码即可高效运转。
...
spec-check:        ## 检查 OpenSpec 变更与 OpenAPI 契约一致性
	pnpm --filter frontend spec:check
generate:          ## 从 OpenAPI 契约生成 TS client 与 Go stub
	pnpm --filter frontend gen:api
	cd apps/backend && oapi-codegen -config config.yaml ../../contracts/openapi.yaml

# ---- 静态检查 ----
lint: lint-frontend lint-backend lint-cli  ## 全量 lint
lint-frontend:     ## ESLint + tsc
	pnpm --filter frontend lint
lint-backend:      ## go vet + golangci-lint
	cd apps/backend && go vet ./... && golangci-lint run
lint-cli:          ## CLI 自身 lint
	pnpm --filter vega-cli lint

# ---- 测试 ----
test: test-frontend test-backend test-cli  ## 全量单元测试
test-frontend:     ## Vitest + RTL
	pnpm --filter frontend test
test-backend:      ## Go test
	cd apps/backend && go test ./...
test-cli:          ## CLI 单元测试
	pnpm --filter vega-cli test
...
# ---- 构建与部署 ----
build: build-frontend build-backend build-cli  ## 全栈构建
build-frontend:
	pnpm --filter frontend build
build-backend:
	cd apps/backend && go build -o ../../dist/server ./cmd/server
build-cli:
	pnpm --filter vega-cli build
...
vega-publish:      ## 发布 vega CLI（需内部 registry）
	pnpm --filter vega-cli publish
```

**关键约定：**
- **对外**：开发者和 Agent 永远只调用 `make <target>`，不直接调用 `pnpm` 或 `go`。
- **对内**：Makefile 通过 `pnpm --filter` 调度 JS 子包，通过 `cd apps/backend && go ...` 调度 Go。
...
| 前端单元/组件 | Vitest + RTL | `make test-frontend` | models、controller、store、表单交互 | 修改工作经历顺序后状态映射正确 |
| 后端服务/API | Go test + httptest + PostgreSQL 测试库 | `make test-backend` | handler 输入输出、鉴权、JSONB 读写 | 更新简历接口拒绝非法 payload |
...
基于 [Fission-AI/OpenSpec](https://github.com/Fission-AI/OpenSpec) 的理念，在进入具体实现前，必须产出并确认“四件套”（通常存放于 `specs/changes/`）：
...
| `specs/archive/` | 功能验收完成 | 归档的 OpenSpec 四件套 | 可检索历史范式 |
...
├── AGENTS.md                           # Agent 规则入口（所有 Agent 通用）
├── CLAUDE.md                           # Claude Code 专属约束（可选）
│
...
│
├── apps/
│   ├── frontend/                       # Vite + React + TS（受 pnpm workspace 管理）
│   │   ├── package.json
│   │   └── src/
│   │       ├── ui/                     # 展示层
│   │       ├── controller/             # 流程编排
...
│   │       └── models/                 # 领域模型
│   │
│   └── backend/                        # Go + Gin（独立 go.mod，不进入 pnpm）
│       ├── go.mod
│       ├── go.sum
│       ├── cmd/server/main.go
│       ├── internal/
...
│       ├── package.json                # 定义 bin: { "vega": "./bin/index.js" }
│       ├── tsconfig.json
│       ├── src/
...
│   └── experiences/                    # 经验沉淀
│
├── generated/                          # 自动生成物（禁止手改）
│   ├── ts-client/                      # orval 生成
│   └── go-stubs/                       # oapi-codegen 生成
│
├── deploy/                             # 部署相关
...
4. **Go 后端骨架**：在 `apps/backend` 初始化 Go + Gin 项目，独立 `go.mod`，确保 `make test-backend` 可独立通过。
5. **前端骨架**：在 `apps/frontend` 初始化 Vite + React，建立五层目录结构。
6. **契约链路**：建立 `specs/changes` 与 `contracts/openapi`，跑通 `proposal / design / spec / tasks` 最小模板。
...
9. **主链路验证**：用 `vega init` 创建一个测试需求，跑通完整 Skill 链路。
10. **归档机制**：每个里程碑执行 `make archive`，把决策、规则、经验回写到 `docs/` 与 Skill 文件。

### 11.3 关键提醒

- **Makefile 是唯一对外入口**：Agent 与开发者均通过 `make <target>` 操作，不要直接调 `pnpm` 或 `go`。
- **pnpm 与 go 物理隔离**：`apps/backend` 不要出现 `package.json`；`packages/vega-cli` 不要出现 `go.mod`。
- **Skill 文件就近分发**：Skill 永远跟随 Agent 配置目录（`.claude/skills/` 等），而不是塞进 `.vega-harness/`。
- **vega CLI 与业务代码物理隔离**：`packages/vega-cli` 独立维护，便于发包；与 `apps/frontend` 共享 lockfile 与工具链。
...
**Harness v7.0 是最具实战指导意义的设计里程碑：**

- **流程层**：回归 v3.1 的 **Skill 体系（10 个原子能力单元） + 需求状态机 + Agent 原生目录分发**，并通过 `.vega-harness/` 沉淀状态与中间产物。
- **工具层**：保留 v4/v5 的 **Node.js `vega` CLI（commander.js）+ pnpm workspace（`apps/frontend` + `packages/vega-cli`）**，让前端开发者无需切换语言生态即可维护流程工具。
- **后端层**：保留 v5 的 **Go 独立模块（`apps/backend` + `go.mod`）**，与 JS 栈通过 `contracts/openapi` 桥接，互不干扰。
- **编排层**：**回归 Makefile** 作为跨语言/异构环境的顶层入口，不再让 `package.json` 越权承担"全栈编排器"角色。

**一句话总结：用 Skill + 状态机驱动需求闭环，用 Node.js `vega` CLI 操作状态机，用 pnpm workspace 管理 JS 生态，用独立 `go.mod` 管理 Go 生态，用根目录 Makefile 把上述一切异构工具链统一为对外的单一命令面；任何一轮交付完成后，把决策、规则和经验归档到 `docs/decisions`、`specs/archive`、`AGENTS.md` 与 Skill 文件中，让下一轮 Agent 在更完整的上下文里继续工作。**
